...
That workflow pins the KAI OS version, uses the deterministic mock provider, writes `artifacts/kaios-verify.json`, writes `artifacts/kaios-run.capsule.json`, and collects `artifacts/kaios-bug-report.json` when the gate fails.
